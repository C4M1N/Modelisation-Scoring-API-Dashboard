...
def prediction():
    ...
    if not check_credentials:
        return Response("Authentication - user not allowed", mimetype='text/html'), 403
...
def check_api_key(request_args):
    ...
    if (
        'loan_app_key' not in response
        or response['loan_app_key'] not in loan_api_keys.values()
    ):
